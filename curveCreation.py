...
for date in curveDates:
    ...
    curveDict[date] = ql.PiecewiseCubicZero(calc_date,
    rate_helpers,
    day_count)

yc = curveDict[dt.date(2024,7,23)]
start = 0
stop = 30
step = 0.25
...
df_1 = pd.DataFrame(list(zip(sequence, points)), columns=["year", "rate"])

fig = px.line(df_1, x='year', y='rate')
fig.show()